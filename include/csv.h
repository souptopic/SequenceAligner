...
typedef struct {
    const char* parts[3];
    size_t lengths[3]; 
    size_t data_pos[2 * READ_CSV_COLS - 2];
    int8_t col_to_data[2 * READ_CSV_COLS + 2];
} Format;
...
INLINE void init_format() {
    ...
    bool write_used[READ_CSV_COLS * 2 + 2] = {false};
    ...
    size_t idx = 0;
    for(size_t i = 0; i < READ_CSV_COLS * 2 + 2 && idx < (READ_CSV_COLS - 1); i++) {
        if(!write_used[i]) {
            fmt.data_pos[idx] = i;
...
#if MODE_WRITE == 1
INLINE char* buffer_output(char* restrict buf_pos, const Data* restrict prev, const Data* restrict curr, const Alignment* restrict result) {
    for (size_t col = 0; col < 4 + (READ_CSV_COLS - 1) * 2; col++) {
        if (col > 0) *buf_pos++ = ',';
        int8_t data_idx = fmt.col_to_data[col];
        if (data_idx >= 0) {
            const char* str = (col == fmt.data_pos[data_idx]) ? prev->other_data : curr->other_data;
            while(*str) *buf_pos++ = *str++;
            continue;
        }
        switch (col) {
            ...
            case WRITE_CSV_ALIGN_POS:
                buf_pos = fast_strcpy(buf_pos, fmt.parts[0], fmt.lengths[0]);
                buf_pos = fast_strcpy(buf_pos, result->seq1_aligned, strlen(result->seq1_aligned));
                buf_pos = fast_strcpy(buf_pos, fmt.parts[1], fmt.lengths[1]);
                buf_pos = fast_strcpy(buf_pos, result->seq2_aligned, strlen(result->seq2_aligned));
                buf_pos = fast_strcpy(buf_pos, fmt.parts[2], fmt.lengths[2]);
                break;
        }
    }
...
INLINE size_t parse_csv_line(char** restrict current, char* restrict seq, char* restrict other_data) {
    char* p = *current;
    char* write_pos = NULL;
    size_t col = 0;
