...
DEFAULT_MULTIPLIER: Final[int] = 1920
UPDATE_INTERVAL_PERCENT: Final[int] = 5
MEMORY_SAFETY_FACTOR: Final[float] = 2.0  # Buffer for other processes

@contextmanager
def progress_bar():
    """Context manager to display a progress bar."""
    try:
        yield
    finally:
        stdout.write('\n')
...
def check_memory_requirements(file_size: int, multiply_factor: int, reverse: bool = True, skip_confirm: bool = False) -> None:
    """Check if system has enough memory for the operation."""
    available_memory = get_available_memory()
    data_multiplier = 2 if reverse else 1
    required_memory = file_size * data_multiplier * multiply_factor * MEMORY_SAFETY_FACTOR
    if not skip_confirm:
...
def enlarge_csv(multiply_factor: int = DEFAULT_MULTIPLIER, 
                input_path: str = None, 
    ...
    with progress_bar():
        for i in range(multiply_factor):
            if not skip_confirm and (i + 1) == next_update:
...
if __name__ == "__main__":
    ...
    # Default paths
    input_path = project_root / 'testing' / 'datasets' / 'avpdb.csv'
    output_path = project_root / 'testing' / 'datasets' / 'avpdb_mega.csv'

    parser = ArgumentParser(description='Create enlarged dataset by multiplying the original data')
    parser.add_argument('multiplier', type=int, nargs='?', default=1920, help='Number of times to multiply the data (default: 1920)')
    parser.add_argument('input_path', type=str, nargs='?', default=input_path, help='Path to input CSV file (default: avpdb.csv)')
    parser.add_argument('output_path', type=str, nargs='?', default=output_path, help='Path to output CSV file (default: avpdb_mega.csv)')
    parser.add_argument('--no-reverse', '-nr', action='store_false', dest='reverse', help='If set, the reversed data will not be appended to the original data')
    parser.add_argument('--no-header', '-nh', action='store_false', dest='skip_header', help='Set if CSV has no header to skip')
    parser.add_argument('--skip-confirm', '-sc', action='store_true', dest='skip_confirm', help='Skip confirmation prompt and remove info messages')
    ...
    try:
        enlarge_csv(args.multiplier, args.input_path, args.output_path, args.reverse, args.skip_header, args.skip_confirm)
