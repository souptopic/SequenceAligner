...
try:
    ...
    from .config_schema import DISPLAY_NAMES, TOOLTIPS, DEFAULT_VALUES, DEFAULT_CHECKBOXES, project_root, validate_config, save_config
except ImportError:
    from build_system import DISPLAY_BINARY_PROFILE, build_env
    from config_schema import DISPLAY_NAMES, TOOLTIPS, DEFAULT_VALUES, DEFAULT_CHECKBOXES, project_root, validate_config, save_config
...
class ConfigEditor:
    ...
    def _init_ui(self):
        ...
        for display, binary, profile in DISPLAY_BINARY_PROFILE:
            ...
            build_env.register_button_callback(f"{binary}_{profile}", lambda en, b=btn: b.configure(state='normal' if en else 'disabled'))
        ...
        for text, cmd in [("Save", self.save), ("Reset", self.reset_defaults), ("Exit", self.root.quit)]:
    ...
    def _browse_file(self, entry, is_input):
        ...
        if filename := (filedialog.askopenfilename if is_input else filedialog.asksaveasfilename)(
                initialdir=current.parent, initialfile=current.name, defaultextension=".csv", filetypes=[("CSV files", "*.csv")]):
            try:
                entry.delete(0, "end")
                entry.insert(0, str(Path(filename).relative_to(project_root)).replace('\\', '/'))
            except ValueError:
    ...
    def save(self):
        ...
        build_env.run_make(self.update_output, "all")
