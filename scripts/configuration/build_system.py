...
class BuildEnvironment:
    def __init__(self):
        ...
        self._build_states: Dict[str, bool] = {"all": False, "tune": False}
    ...
    @staticmethod
    @lru_cache(maxsize=1)
    def get_msys2_paths() -> Dict[str, str]:
        return {
    ...
    def verify(self) -> Tuple[bool, Optional[str]]:
        ...
        if not any(Path(path).exists() for path in self.get_msys2_paths().values()):
    ...
    def _handle_process_output(self, process: subprocess.Popen, output_fn: ProcessCallback) -> bool:
        try:
            while True:
    ...
    def _run_process(self, cmd: str, output_fn: ProcessCallback, cwd: Path, on_complete: Optional[StateCallback] = None, process_key: Optional[str] = None) -> None:
        try:
    ...
    def run_make(self, output_fn: ProcessCallback, target: str, cwd: Path = project_root, on_complete: Optional[StateCallback] = None) -> None:
        with self._lock:
    ...
    def run_binary(self, output_fn: ProcessCallback, name: str, cwd: Path = project_root) -> None:
    ...
    def build_and_run(self, output_fn: ProcessCallback, binary: str, target: str, cwd: Path = project_root) -> None:
        if self.is_busy():
            return
    ...
    def register_button_callback(self, _: str, callback: StateCallback) -> None:
    ...
    def init(self, output_fn: ProcessCallback, cwd: Path = project_root) -> None:
